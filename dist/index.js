...
async function runSync(groupKeys, onProgress) {
  ...
  if (s.confirmAbove > 0 && ids.length > s.confirmAbove && !window.confirm(`Sync settings to ${ids.length} photos?`)) return null;
  const groups = GROUPS.filter((g) => groupKeys.has(g.key));
  let geoSkipped = 0;
  const errors = await forEachTarget(ids, sourceId, async (id, cur) => {
    const photo = byId.get(id);
    const sizeDiffers = s.geometrySafety && srcPhoto && photo && (photo.width !== srcPhoto.width || photo.height !== srcPhoto.height);
    let merged;
...
function Btn({ primary, on, children, off }) {
  ...
  return /* @__PURE__ */ h("button", { style: off ? disabled(base) : base, disabled: !!off, onClick: on }, children);
}
function BatchPanel() {
